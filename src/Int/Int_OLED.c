...
void Int_OLED_WriteData(u8 bytes[], u8 length)
{
    u8 i;
    Dri_IIC_Start();
    Dri_IIC_SendByte(DEV_ADDR); // 发送设备地址+写位
    Dri_IIC_ReceiveAck();
    Dri_IIC_SendByte(DATA_MODE); // 发送数据模式
    Dri_IIC_ReceiveAck();
    for (i = 0; i < length; i++) {
        Dri_IIC_SendByte(bytes[i]); // 发送数据字节
        Dri_IIC_ReceiveAck();
    }
    Dri_IIC_Stop();
}
...
void Int_OLED_WriteCmd(u8 bytes[], u8 length)
{
    ...
    Dri_IIC_SendByte(DEV_ADDR); // 发送设备地址+写位
    Dri_IIC_ReceiveAck();
    Dri_IIC_SendByte(CMD_MODE); // 发送数据模式
    Dri_IIC_ReceiveAck();
    for (i = 0; i < length; i++) {
        Dri_IIC_SendByte(bytes[i]); // 发送数据字节
        Dri_IIC_ReceiveAck();
...
void Int_OLED_Clear()
{
    ...
    u8 data_bytes[8]={0};
    u16 i;
    cmd_bytes[0] = 0x20; // 设置内存地址模式
    cmd_bytes[1] = 0x00; // 水平地址模式
